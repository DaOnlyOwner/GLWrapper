...
namespace doo
{
	namespace gl
	{
		...
		std::pair<u32, u32> MeshManager::AddStaticGeometry(const std::vector<Vertex>& p_vertices,
		                                                   const std::vector<u32>& p_indices)
		{
		...
		void MeshManager::EndStaticGeometry()
		{
			...
			CreateVBOFromVector(m_verticesStaticGeometry, m_indicesStaticGeometry, layout.GetInterleavedLayoutInfo(),
			                    layout.GetNumAttribPtrs(), GL_STATIC_DRAW);
